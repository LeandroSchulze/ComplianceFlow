...
class ScanRequest(BaseModel):
    cliente_nombre: str
    norma_id: str  # Recibe de forma obligatoria la reglamentación seleccionada
...
DATA_ESTANDARES = {
    ...
    "SOC2-S3": {"norma": "SOC 2 Type II — CC6.3", "titulo": "Análisis Criptográfico S3", "evidencia_id": "EV-SOC2-S3-9202", "estado": "🟢 100% CUMPLIDO", "detalle": "Public Access Block activo y cifrado SSE-S3 de forma persistente."},[cite: 5]
...
def crear_preferencia_individual():
    ...
    try:
        ...
        preference_data = {
            ...
            "back_urls": {
                "success": "https://www.complianceflow.me/dashboard?payment=success",
                "failure": "https://www.complianceflow.me/dashboard",
                "pending": "https://www.complianceflow.me/dashboard"
            },
...
def crear_preferencia_premium():
    ...
    try:
        ...
        preference_data = {
            ...
            "back_urls": {
                "success": "https://www.complianceflow.me/dashboard?tier=premium",
                "failure": "https://www.complianceflow.me/dashboard",
                "pending": "https://www.complianceflow.me/dashboard"
            },
            "auto_return": "approved"
...
# --- ENDPOINT DE ESCANEO PÚBLICO CON CONTROL DE REGISTROS ---
@app.post("/api/compliance/scan", tags=["Escáner"])
def ejecutar_escaneo_web(solicitud: ScanRequest, request: Request):
    ip_cliente = obtener_ip_cliente(request)[cite: 5]
    
    # 1. Validar si la IP actual superó el umbral en Postgres[cite: 5]
    with obtener_conexion_db() as conn:[cite: 5]
        with conn.cursor() as cursor:
            ...
            if resultado and resultado[0] >= 3:
                raise HTTPException(
                    status_code=402, 
                    detail="Límite freemium superado. Registre su cuenta corporativa para continuar."
                )
                
    # 2. Si pasa, actualizamos e incrementamos la métrica por IP[cite: 5]
    verificar_y_actualizar_limite_ip(ip_cliente)
    
    # 3. Mapear dinámicamente los datos según la reglamentación elegida[cite: 5]
    info_norma = DATA_ESTANDARES.get(solicitud.norma_id, DATA_ESTANDARES["ISO-IAM"])[cite: 5]
    datos_reporte = {
        "id": solicitud.norma_id,
        "norma": info_norma["norma"],
        "titulo": info_norma["titulo"],
        "evidencia_id": info_norma["evidencia_id"],
        "estado": info_norma["estado"],
        "detalle": info_norma["detalle"]
    }
    
    reporter = ReportGenerator(cliente_nombre=solicitud.cliente_nombre)[cite: 5]
    archivo_pdf = reporter.generar_pdf_cumplimiento(datos_reporte)[cite: 5]
